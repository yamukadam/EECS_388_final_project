...
while(1):
 ...
 deg = rad2deg(rad)
 # Your code here in this if condition -- The if condition is used to sample
the every 4 data for sending.
 if count%4 == 0:
 # Your code here.
 temp = str(int(deg)) + "\n"
 ser1.write(bytes(temp))
 print("sent=" + temp)
 ...
 #print('pred: {:0.2f} deg. took: {:0.2f} ms | cam={:0.2f} prep={:0.2f}
pred={:0.2f}'.format(deg, tot_time, cam_time, prep_time, pred_time))
...
print('completed inference, total frames: {}, average fps: {}
Hz'.format(curFrame+1, round(fps, 1)))
